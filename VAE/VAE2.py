...
class ResNetCNN(nn.Module):
    def __init__(self):
        ...
        # Additional layers to reduce features to 128
        self.reduction_layers = nn.Sequential(
            nn.Conv2d(512, 256, kernel_size=3, padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(256, 128, kernel_size=3, padding=1),  # Output size is now 128
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d((1, 1))
        )

        # Layers to compute mean and variance for the KL loss
        self.mean_layer = nn.Linear(128, 128)  # Linear layer for mean
        self.var_layer = nn.Linear(128, 128)  # Linear layer for variance
    ...
    def compute_triplet_loss(self, anchor_latent, positive_latent):
        # Compute the Euclidean distance between anchor and positive latent vectors
        distance = torch.nn.functional.pairwise_distance(anchor_latent, positive_latent)
        
        # Use the distance as the loss
...
class VAE(nn.Module):
    def __init__(self,z_dim = 128 ,*args, **kwargs):
        ...
        # Decoder
        self.dec_fc1 = nn.Linear(128 + 128, 128 * 8 * 8 * 8)
    ...
    def forward(self, modeldata, latent_cnn_mean, latent_cnn_var):
        mu_voxel, logvar_voxel = self.encode(modeldata)
        z_vae = self.reparameterize(mu_voxel, logvar_voxel)
        
        z_cnn = self.reparameterize(latent_cnn_mean, latent_cnn_var)
        combined_latent = torch.cat((z_vae, z_cnn), dim=1)
    ...
    def loss2(self, inputs, outputs):
        ...
        outputs_clip = torch.clip(outputs, 1e-7, 1.0 - 1e-7)
        loss = -(70.0 * inputs * torch.log(outputs_clip) +  30 * (1.0 - inputs) * torch.log(1.0 - outputs_clip))
        return loss.mean()
